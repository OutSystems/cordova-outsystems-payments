 ...
 *
 * This script is always executed from the `main` clone (never from PR-controlled
 * code). It reads PR metadata via environment variables (no shell interpolation)
 * and writes outputs to the file pointed to by $GITHUB_OUTPUT.
 *
 * Required env vars:
 *   PR_TITLE        PR title
 ...
 *   MAIN_DIR        Path to the main clone (read-only baseline)
...
function latestChangelogVersion(content) {
  const m = content.match(/^##\s+(\d+\.\d+\.\d+)\s*$/m);
  return m ? m[1] : null;
}
...
function buildChangelogEntry(version, bump, parsed) {
...
async function main() {
  ...
  if (bump === 'none') {
    ...
    newChangelog = rebuildChangelog(
      mainChangelog,
      buildChangelogEntry(targetVersion, bump, parsed),
    );
